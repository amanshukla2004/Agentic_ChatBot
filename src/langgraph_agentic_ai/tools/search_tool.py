...
def get_weather(location: str) -> str:
    ...
    try:
        ...
        response = requests.get(f"https://wttr.in/{location}?format=3", timeout=3)
        response.raise_for_status()
        return response.text
    except requests.exceptions.Timeout:
        return f"Weather service timed out. Please try asking again later, or rephrase to use general web search."
...
def youtube_search(query: str, max_results: int = 5) -> str:
    """Search YouTube for a specific topic or video.
    
    Args:
        query: The search query.
        max_results: The number of video results to return. Defaults to 5.
    """
    yt = YouTubeSearchTool()
    result_str = yt.run(f"{query}, {max_results}")
...
def wikipedia_search(query: str, max_results: int = 2) -> str:
    """Search Wikipedia for a specific topic, person, or concept.
    
    Args:
        query: The search query.
        max_results: Number of results to return. Defaults to 2.
    """
    api_wrapper = WikipediaAPIWrapper(top_k_results=max_results, doc_content_chars_max=1000)
